...
/**
  * This learning algorithm simply counts the number of times that each word
  * appears in training data associated with each possible part of speech tag.
  * It assumes that its feature extractor classifier is <code>discrete</code>,
  * i.e. that in the LBJ source file, a single <code>discrete</code>
  * classifier is named in the learning classifier expression's
  * <code>using</code> clause.  That classifier is assumed to return the form
  * of the word.
  *
  * <p> <b>Note:</b> This learner will not work if feature pre-extraction is
  * enabled for it.
  *
  * @author Nick Rizzolo
 **/
public class POSBaselineLearner extends Learner
{
  ...
  public POSBaselineLearner() { this(""); }
  ...
  public POSBaselineLearner(Parameters p) { this(""); }
   ...
   **/
  public Learner emptyClone() { return new POSBaselineLearner(); }
  ...
  /**
    ...
    * <code>learn(int[],double[],int[],double[])</code> signature
    * for the compiler. Not to be used by the programmer.
    *
  ...
  public void forget() { table.clear(); }
  ...
  public static boolean looksLikeNumber(String form) {
    ...
    for (int i = 0; i < form.length(); ++i) {
      if (Character.isDigit(form.charAt(i))) containsDigit = true;
      else if (".,-".indexOf(form.charAt(i)) == -1) return false;
    }
  ...
  /**
    * Computes the prediction for this example.  This method only exists
    * because of the way that generated code calls
    * {@link #discreteValue(Object)}, {@link #featureValue(Object)}, and
  ...
  protected String computePrediction(Object example) {
    ...
    if (counts == null) {
      if (form.equals(";")) l = ":";
      else if (looksLikeNumber(form)) l = "CD";
      else l = "UNKNOWN";
    }
    else {
      int best = 0;
  ...
  public Feature featureValue(Object example) {
    return
      new DiscretePrimitiveStringFeature(
            containingPackage, name, "", computePrediction(example));
  }


  /**
    * This function makes one or more decisions about a single object,
    * returning those decisions as <code>Feature</code>s in a vector.
    *
  ...
  public ScoreSet scores(int[] features, double[] values) { return null; }
  ...
  {
    if (!table.containsKey(form)) return 0;
    int result = 0;
  ...
  public Set<String> allowableTags(String form)
  {
    if (!table.containsKey(form))
    {
      ...
      if (form.equals(";")) result.add(":");
      else if (looksLikeNumber(form)) result.add("CD");
      return result;
    }
                           ...
                           HashMap<String, TreeMap<String, Integer>> table)
  {
    ...
    for (String key : keys) {
      ...
      for (int j = 0; j < tags.length; ++j) indexes[j] = j;
      ...
      for (Integer indexe : indexes) out.print(" " + tags[indexe] + "(" + values[indexe] + ")");
      out.println();
    }
  }
  ...
  /**
    * Writes the binary representation of the specific form of map used by
    * this class.
    *
    * @param out  The output stream.
  ...
  /**
    * Reads in the binary representation of the specific form of map used by
    * this class.
    *
    * @param in   The input stream.
                             ...
                             HashMap<String, TreeMap<String, Integer>> map)
  {
    ...
    for (int i = 0; i < tableSize; ++i) {
      ...
      TreeMap<String, Integer> tableValue = new TreeMap<String, Integer>();
