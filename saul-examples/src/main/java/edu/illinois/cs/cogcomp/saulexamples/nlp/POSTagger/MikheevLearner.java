 ...
 * Only rules involving all-letter suffixes of lengths 3 and 4 of words whose
 * length is at least 2 greater than the suffix are extracted.  The learned
 * rules are divided into three categories: words that are capitalized and
 * appear first in the sentence, words that are capitalized and don't appear
 * first in the sentence, and uncapitalized, unhyphenated words.
 * Uncapitalized, hyphenated words are hard coded to a rule that produces the
 * set <code>{ NN, JJ }</code>.
 *
 * Ported from illinois-pos, with small modifications to make it compatible with
 * `TextAnnotation` and `Constiutnet`, instead of `Token` and `Word`.
 *
 * @author Nick Rizzolo
 **/
...
public class MikheevLearner extends POSBaselineLearner
{
    ...
    public MikheevLearner(Parameters p) { this(""); }
     ...
     **/
    public Learner emptyClone() { return new MikheevLearner(); }
     ...
     **/
    private void increment(HashMap<String, TreeMap<String, Integer>> table,
                           String suffix, String tag) {
    ...
    public void prune(HashMap<String, TreeMap<String, Integer>> table) {
        for (Iterator<Map.Entry<String, TreeMap<String, Integer>>> I =
            ...
            for (Iterator<Map.Entry<String, Integer>> J =
    ...
    /**
     * Returns the set of tags that the given word's suffix has been observed
     * with, or a reasonable default if the suffix has never been observed.
     *
     * @param word The word.
     * @return The set of tags observed in association with the given word's
     *         suffix, or a reasonable default if the suffix has never been
     *         observed.
     **/
    public Set<String> allowableTags(Constituent word) {
        ...
        if (isCapitalized(wordForm)) {
            ...
            if (result.size() == 0) result.add("NN");
        }
    ...
    public Constituent nearestBefore(Constituent word) {
        IQueryable<Constituent> constituentsInWindow = word.getTextAnnotation().select(word.getViewName());
        IQueryable<Constituent> constituentsBefore = constituentsInWindow.where(Queries.before(word));
        constituentsBefore.orderBy(new Comparator<Constituent>() {
            @Override
        ...
        Constituent nearestConstituent = null;
        Iterator<Constituent> it = constituentsBefore.iterator();
        if (it.hasNext()) {
            nearestConstituent = it.next();
        }
        return nearestConstituent;
    }
}