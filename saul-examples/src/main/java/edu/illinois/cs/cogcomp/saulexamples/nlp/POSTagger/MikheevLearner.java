...
public class MikheevLearner extends POSBaselineLearner
{
  ...
  public void learn(Object example) {
    ...
    if (form.length() >= 5) {
      ...
      if (allLetters) {
        ...
        if (w.capitalized) {
          if (w.previous == null) t = firstCapitalized;
  ...
  public Set<String> allowableTags(Word word) {
    ...
    if (word.capitalized) {
      ...
      if (word.previous == null) t = firstCapitalized;
      else t = notFirstCapitalized;

      if (word.form.length() >= 6) {
        String suffix =
          word.form.substring(word.form.length() - 4).toLowerCase();
      ...
      if (result.size() == 0 && word.form.length() >= 5) {
        String suffix =
          word.form.substring(word.form.length() - 3).toLowerCase();
        if (t.containsKey(suffix))
          result = t.get(suffix).keySet();
      }
