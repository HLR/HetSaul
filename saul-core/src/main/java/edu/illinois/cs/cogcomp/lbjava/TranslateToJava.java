...
import edu.illinois.cs.cogcomp.lbjava.io.HexOutputStream;
...
public class TranslateToJava extends Pass
{
	...
	public static final String disclaimer =
			"// Modifying this comment will cause the next execution of LBJava to "
					+ "overwrite this file.";
	/**
	...
	private static final String[] noOverride =
		...
		"public edu.illinois.cs.cogcomp.lbjava.classify.Feature featureValue(java.lang.Object a0)",
	...
	/**
	 * Lets {@link edu.illinois.cs.cogcomp.lbjava.IR.VariableDeclaration}s know if they are contained in the
	 ...
	 * create {@link edu.illinois.cs.cogcomp.lbjava.infer.Constraint} objects rather than computing the
	 ...
	 * occupies in an {@link edu.illinois.cs.cogcomp.lbjava.infer.EqualityArgumentReplacer}'s vector; the keys of
	 * the map are names of quantification variables, and the values are
	 * <code>Integer</code>s.
	 **/
	private HashMap<String, Integer> quantificationVariables;
	/**
	 * Lets AST children know the index that a given context variable occupies
	 * in an {@link edu.illinois.cs.cogcomp.lbjava.infer.EqualityArgumentReplacer}'s vector; the keys of the map are
	 * names of context variables, and the values are <code>Integer</code>s.
	 **/
	private HashMap<String, Integer> contextVariables;
	/**
	 ...
	 * corresponding to the specified {@link edu.illinois.cs.cogcomp.lbjava.CodeGenerator}.
	 *
	 ...
	 * {@link edu.illinois.cs.cogcomp.lbjava.learn.Learner} to check types and call themselves on the
	 ...
	 * {@link edu.illinois.cs.cogcomp.lbjava.learn.Learner} down to the super class of this learner are
	 * overridden to call the super class's implementation on the unique
	 * instance.
	 *
	 * @param out  The stream to write to.
	 * @param lce  The {@link edu.illinois.cs.cogcomp.lbjava.IR.LearningClassifierExpression} representing the
	 *             learner.
	 **/
	...
	public static void generateLearnerBody(PrintStream out,
		...
		tabs = "\t\t\t";
		out.println(tabs + "lcFilePath = new java.net.URL(\"file:\" + "
				+ "modelPath);");
		out.println(tabs + "lexFilePath = new java.net.URL(\"file:\" + "
				+ "lexiconPath);");
		tabs = "\t\t";
		out.println(tabs + "}");
		...
		tabs = "\t\t";
		out.println(tabs + "}");
		out.println(tabs + "else if (IOUtilities.existsInClasspath(" + lceName + ".class, modelPath)) {");
		tabs = "\t\t\t";
		out.println(tabs + "readModel(IOUtilities.loadFromClasspath(" + lceName + ".class, modelPath));");
		out.println(tabs + "readLexiconOnDemand(IOUtilities.loadFromClasspath(" + lceName + ".class, lexiconPath));");
		tabs = "\t\t";
		out.println(tabs + "}");
		out.println(tabs + "else {");
		tabs = "\t\t\t";
		out.println(tabs + "containingPackage = \""
				+ AST.globalSymbolTable.getPackage() + "\";");
		...
		out.println("      testParser = "
				+ "edu.illinois.cs.cogcomp.lbjava.util.ClassUtils.getParser(testParserName, new "
		...
		for (int i = 0; i < methods.length; ++i) {
			...
			String sig = signature(methods[i], modifiers, returned, name, parameters);
	 ...
	 * method.
	 *
	 * @see #generateLearnerBody(java.io.PrintStream, edu.illinois.cs.cogcomp.lbjava.IR.LearningClassifierExpression)
	...
	/**
	 * Generate code that overrides the methods of {@link edu.illinois.cs.cogcomp.lbjava.classify.Classifier} that
	...
	/**
	 * Generates code that overrides the {@link edu.illinois.cs.cogcomp.lbjava.classify.Classifier#classify(Object[])}
	 ...
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.InferenceDeclaration}.
	 *
	 * @param out  The stream to write to.
	 ...
	 * classifiers' {@link edu.illinois.cs.cogcomp.lbjava.classify.Classifier#classify(Object)} methods that checks to
	 * see if that input <code>Object</code> has the appropriate type.
	 *
	 * @param indent       The whitespace indentation in the generated code.
	 * @param name         The name of the {@link edu.illinois.cs.cogcomp.lbjava.CodeGenerator} whose input is
	 *                     being checked.
	 * @param type         The type of {@link edu.illinois.cs.cogcomp.lbjava.CodeGenerator} whose input is
	 *                     being checked (capitalized).
	 * @param array        Whether or not the method being type checked takes
	 *                     an array of the input type.
	 * @param input        The correct input type of the {@link edu.illinois.cs.cogcomp.lbjava.CodeGenerator}.
	 * @param line         The line number on which the {@link edu.illinois.cs.cogcomp.lbjava.CodeGenerator}
	 *                     appears.
	 ...
	 * have either type {@link edu.illinois.cs.cogcomp.lbjava.classify.Feature} or
	 * {@link edu.illinois.cs.cogcomp.lbjava.classify.FeatureVector} as appropriate.
	 *
	...
	/**
	 * Compress the textual representation of an {@link edu.illinois.cs.cogcomp.lbjava.IR.ASTNode}, convert to
	...
	/**
	 * Code is only generated for a {@link edu.illinois.cs.cogcomp.lbjava.IR.ClassifierName} when it is the only
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.ClassifierExpression} on the right hand side of the arrow (and
	...
	public void run(CodedClassifier cc) {
		...
		HashSet<String> invoked = SemanticAnalysis.invokedGraph.get(ccName);
		if (invoked != null && invoked.size() > 0) {
			for (Iterator<String> I = invoked.iterator(); I.hasNext(); ) {
				String name = I.next();
				String nameNoDots = name.replace('.', '$');
				out.println("  private static final " + name + " __" + nameNoDots
						+ " = new " + name + "();");
		...
		Type input = cc.argument.getType();
	...
	public void run(CompositeGenerator cg) {
		...
		Type input = cg.argument.getType();
	...
	public void run(LearningClassifierExpression lce) {
		...
		if (lce.parameterSets.size() > 0) {
			for (ListIterator<ParameterSet> I = lce.parameterSets.listIterator(); I.hasNext(); ) {
				ParameterSet e = I.next();
	...
	public void run(ConstraintDeclaration cd) {
		...
		HashSet<String> invoked = SemanticAnalysis.invokedGraph.get(cdName);
		if (invoked != null && invoked.size() > 0) {
			for (Iterator<String> I = invoked.iterator(); I.hasNext(); ) {
				String name = I.next();
				String nameNoDots = name.replace('.', '$');
		...
		Type input = cd.argument.getType();
		out.println("  public String getInputType() { return \""
				+ input.typeClass().getName() + "\"; }\n");
	...
	public void run(ConstraintStatementExpression e) {
		...
		if (constraintMode && e.constraint.containsQuantifiedVariable()) {
			...
			int i = 0;
			HashSet<Argument> referenced = e.constraint.getVariableTypes();
			for (Iterator<Argument> I = referenced.iterator(); I.hasNext(); ) {
				Argument a = I.next();
				Type t = a.getType();
	...
	private void generateReplacerMethodEnvironment(Expression expression) {
		for (Iterator<Argument> I = expression.getVariableTypes().iterator(); I.hasNext(); )
		{
			Argument a = I.next();
			Type type = a.getType();
			String primitiveTypeName = null;
	 ...
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.ConstraintEqualityExpression} into the appropriate method of an
	 * {@link edu.illinois.cs.cogcomp.lbjava.infer.EqualityArgumentReplacer}.
	 *
	 ...
	 * variable from a {@link edu.illinois.cs.cogcomp.lbjava.IR.ConstraintEqualityExpression} into an argument of
	 * a {@link edu.illinois.cs.cogcomp.lbjava.infer.FirstOrderEquality}.
	 *
	 ...
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.ConstraintEqualityExpression} into an argument of a
	 * {@link edu.illinois.cs.cogcomp.lbjava.infer.FirstOrderEquality}.
	 *
	 * @param expression   The expression.
	...
	/**
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.UniversalQuantifierExpression}s and
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.ExistentialQuantifierExpression}s generate their code through
	 * this method.
	 *
	 * @param e  The node to process.
	 **/
	...
	/**
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.AtLeastQuantifierExpression}s and
	 * {@link edu.illinois.cs.cogcomp.lbjava.IR.AtMostQuantifierExpression}s generate their code through this
	 * method.
	 *
	 * @param e  The node to process.
	 **/
	...
	public void run(Name n) {
		...
		if (currentCG != null && n.name.length > 1) {
			HashSet<String> invoked = SemanticAnalysis.invokedGraph.get(currentCG.getName());
			if (invoked != null) {
