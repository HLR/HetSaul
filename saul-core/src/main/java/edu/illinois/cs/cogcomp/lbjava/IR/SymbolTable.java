...
public class SymbolTable
{
	...
	private HashSet<String> imported;
	...
	public Class<?> classForName(Name name) {
		...
		for (Iterator<String> I = imported.iterator(); I.hasNext(); ) {
		...
		for (Iterator<String> I = prefixes.iterator(); I.hasNext() && result == null; ) {
			...
			File javaFile = findFile(fqName, "java", Main.sourcePath);

			if (javaFile != null) {
				File classFile = findFile(fqName, "class", Main.classPath);

				if ((classFile == null
						|| javaFile.lastModified() > classFile.lastModified())
						&& Train.runJavac(javaFile.toString()))
					System.exit(1);
			}
			...
			for (int i = 0; i < name.name.length - 1 && result == null; ++i) {
				...
				javaFile = findFile(fqName, "java", Main.sourcePath);
	...
	public void print(String indent) {
		...
		if (children.size() > 0) {
			for (Iterator<SymbolTable> I = children.iterator(); I.hasNext(); ) {
				System.out.println();
