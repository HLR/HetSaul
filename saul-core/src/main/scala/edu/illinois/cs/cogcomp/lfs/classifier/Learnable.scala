...
import edu.illinois.cs.cogcomp.lfs.TestContinuos
import edu.illinois.cs.cogcomp.lfs.data_model.DataModel
import edu.illinois.cs.cogcomp.lfs.data_model.attribute.{Attribute, AttributeWithWindow, CombinedDiscreteAttribute, RelationalFeature}
...
abstract class Learnable[T <: AnyRef](val datamodel : DataModel)(implicit tag : ClassTag[T]) extends LBJLearnerEquivalent{
 ...
 // var net:OVector
  ...
  val classifier : Learner= {
    ...
    if (algorithm.equals("SparsePerceptron"))
    {
...
def testContinuos(testData:Iterable[T]):Unit={
  ...
  testReader.reset()
  val tester=new TestContinuos(classifier,classifier.getLabeler(),testReader, true)
 // tester.printPerformance(System.out)
  //val ret = tester.getLabels.map({
   // label => (label , (tester.getF1(label),tester.getPrecision(label),tester.getRecall(label)))
 // })
  ...
  def crossValidation(k : Int) = {
    ...
    def printTestResult(result : (String,(Double,Double,Double))) : Unit = { result match{
